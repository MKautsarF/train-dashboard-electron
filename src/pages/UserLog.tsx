...
const UserLog = () => {
  const [DeleteAllSubmission, setDeleteAllSubmission] = useState(false);
  ...
  const deleteAllSubmissionButton = async () => {
    try {
      setDeleteAllSubmission(false);
  ...
  return (
    <>
      ...
      <Dialog
        open={DeleteAllSubmission}
        onClose={() => setDeleteAllSubmission(false)}
      >
        <DialogContent className="min-w-[260px]">
          Menghapus semua log submission milik: "{userLog?.name}"?
        </DialogContent>
        <DialogActions className="flex mb-2 justify-between">
          <Button
            className="mx-2"
            onClick={() => setDeleteAllSubmission(false)}
            color="primary"
          >
            Tidak
          </Button>
          <Button
            className="mx-2"
            onClick={() => deleteAllSubmissionButton()}
            variant="contained"
            color="error"
          >
            Ya
