...
// import { default as krlJson } from '@/static/MockJSON_KRL.json';
...
const StartKRL: React.FC = () => {
  ...
  // const [motionDisplay, setMotionDisplay] = useAtom(motionStatusAtom);

  // const [simulation, setSimulation] = useState(true);
  const jsonPath = 'C:/Train Simulator/Data/MockJSON_KRL.json';
  const krlJson = JSON.parse(fs.readFileSync(jsonPath, 'utf-8'));
  ...
  useEffect(() => {
    ...
    socketClients.forEach((socket) => {
      socket.on('data', (data) => {
        ...
        if (dataUE.nilai) {
          console.log('KRL score data: ', dataUE);
  ...
  return (
    <Container w={1000}>
      ...
      <div className="p-8">
        <h1 className="w-full text-center mb-8">Penilaian Kereta: KRL</h1>
        ...
        <section>
          {/* <Box component="form" id="penilaian-form" onSubmit={handleFinish}>
            <h2>Unit Kompetensi 1: {krlJson.penilaian.judul}</h2>
            {krlJson.penilaian.data.map((data: any, idx: number) => (
              <LangkahKerja key={idx} keyVal={idx} data={data} />
            ))}
          </Box> */}
          <Box component="form" id="penilaian-form" onSubmit={handleFinish}>
            <div>
              {krlJson.penilaian.map((nilai: any, i: number) => (
      ...
      <Dialog open={notesOpen} onClose={() => setNotesOpen(false)}>
        ...
        <DialogActions>
          ...
          <Button type="submit" form="penilaian-form">
            Selesai
          </Button>
        </DialogActions>
      </Dialog>
